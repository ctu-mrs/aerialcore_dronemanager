...
class DroneManager:
    ...
    def __init__(self):
        ...
        self.trajectory_file_2_ = rospy.get_param('~trajectory_file_2')
        ...
        # the trajectory object for publishing -- Second Subtrajectory
        self.trajectory_2_ = TrajectoryReferenceSrvRequest()

        self.trajectory_2_.trajectory.header.frame_id = self.reference_frame_
        self.trajectory_2_.trajectory.fly_now = False # cause we will call goto
        self.trajectory_2_.trajectory.dt = 1.0/self.trajectory_sampling_rate_
        self.trajectory_2_.trajectory.use_heading = True
        ...
        # Read the second subtrajectory      
        with open(self.trajectory_file_2_, newline='') as csvfile:
            ...
            for row in reader:
                ...
                point_2.position.x = float(row[0])
                point_2.position.y = float(row[1])
                point_2.position.z = float(row[2])
                point_2.heading    = float(row[3])

                self.trajectory_2_.trajectory.points.append(point_2)
...
## | ------------------------- timers ------------------------- |
    ...
    def timerMain(self, event):
        ...
        rospy.sleep(0.1)   
        rospy.loginfo('waiting for refilling operation') 
        rospy.loginfo('sleeping for additional {} s'.format(self.refilling_duration_))
        rospy.sleep(self.refilling_duration_) # Waiting for refilling time
            
        rospy.sleep(0.1)      
        rospy.loginfo_once("the UAV can track the next part")  
        ...
        self.setTrajectory(self.trajectory_2_)
        ...
        # #{ In case you want to come back home
        #
        # while not self.freeToCommand():
        #    rospy.sleep(0.1)
        #    rospy.loginfo('waiting for the UAV to stop')
        ...
        #
        # #} end in case you want to come back home 

        # #{ Disabling the take-off
        #
        # self.takeoff()
        #
        # #}
        ...
        # ----------------------------------------------------------------------------
        # |        the sequencial functional chart should be placed in here  /\      |
        # ---------------------------------------------------------------------------
